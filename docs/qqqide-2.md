...
V11 转向纯本地机械筛——每层楼完结时自动把对话摘要成 Q/A/工具行，零网络零费用。V12 修复了 splice 导致的前缀缓存断裂。V13 引入双包装盒体系（温柔盒 + 绝对盒）和阀值压缩。V14 修复了 biscuit 重排序带来的缓存全断 bug。V15 引入三按钮压缩体系 + fx 记忆区域。V16 强制 biscuit 唯一性（去重→先合并再删），修复 Z 消息阻断导致楼层丢失。V17 二次恢复防线——修复切走 quest 再切回时上下文全丢的恶性 bug。
