...
function writeMacLaunchers(unpacked) {
  ...
  const readme = [
    ...
    '',
    '【系统授权】（首次使用一次，同一签名证书升级后无需再设）',
    '窗口编队热键（Space+1/2/q/w/a/s/z/x）与窗口布局功能需要两项权限：',
    '  系统设置 → 隐私与安全性 → 输入监控   → 勾选 qqqide',
    '  系统设置 → 隐私与安全性 → 辅助功能   → 勾选 qqqide',
    '（列表里没有 qqqide 时：点 + 添加 qqqide.app）',
    '',
    '【终端】kmd 标签页支持 zsh / bash；gmd 为 Windows 专属（mac 自动隐藏）。',
    '【快捷键】键盘组合沿用 Ctrl（mac 上 Cmd 等效）；编队召回 = 按住空格再按槽位键。',
    ''
  ].join('\n');
  try {
