...
var AgentLoop = (function () {
    ...
    EnvelopeStripper.prototype.finalize = function () {
        ...
        // ★ 剥离 DeepSeek ReAct 格式的文本工具调用（模型在无原生 function calling 时输出）
        // 匹配: Action: tool_name\nAction Input: {...}\n
        if (/^Action:\s*\w+/m.test(cleanContent)) {
            cleanContent = cleanContent.replace(/^Action:\s*\w+.*(?:\nAction Input:\s*\{[\s\S]*?\}\n?)?/gm, '');
            cleanContent = cleanContent.replace(/^Action Input:\s*\{[\s\S]*?\}\n?/gm, '');
            cleanContent = cleanContent.replace(/\x0a{3,}/g, '\x0a\x0a').replace(/^\x0a+/, '').trim();
        }
    ...
    function AgentLoop(opts) {
        ...
        // ★ 超时血条：当前请求的起止时间（panel-clock 读取渲染）
        this._requestStartPerf = 0;
        this._requestDeadlineMs = 0;  // 0=无进行中的请求
        // 计费
    ...
    AgentLoop.prototype._repairOrphanedToolCalls = function () {
        ...
        for (var pass = 0; pass < 20; pass++) {  // ★ 20 轮扫描，覆盖长对话多组孤儿 tool_calls
            var cutAt = -1;
    ...
    AgentLoop.prototype._callGateway = async function (messages, opts) {
        ...
        for (var retry = 0; retry <= MAX_RETRIES; retry++) {
            ...
            try {
                ...
                self._requestStartPerf = _fetchStart;
                self._requestDeadlineMs = _deadlineMs;
                ...
                self._requestDeadlineMs = 0;  // ★ 超时血条：收到响应，隐藏血条
                ...
                if (err.name === 'AbortError') {
                    ...
                    self._requestDeadlineMs = 0;  // ★ 超时血条：请求结束，隐藏血条
