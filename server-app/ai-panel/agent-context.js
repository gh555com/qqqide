...
; (function () {
    ...
    AgentLoop.prototype._rebuildBackpack = async function () {
        ...
        try {
            ...
            // ★ 2026-09-18 q279 重启僵尸实报根治 + 2026-09-19 合并式收敛：清零必须同步写盘（只清内存不写盘 →
            //   重启恢复旧实报复活成僵尸，f117 末 215K / f118 开局实锤）。写盘唯一入口 _questMetaPatch——
            //   questStore.save 为整行替换语义，只传 3 个字段会把 totalCostGe/currentFloorNum 等其余字段整体铲掉。
            if (self._questId && typeof _questMetaPatch === 'function') {
                try { _questMetaPatch(self._questId, { lastApiPromptTokens: 0, lastApiTotalTokens: 0, lastApiCompletionTokens: 0 }); } catch (_) { }
            }
