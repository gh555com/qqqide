...
function _a4BuildCompleteFloorPayload(ag, floorNum, opts) {
    ...
    var payload = {
        ...
        byokUsed: !!ag._floorByokUsed,          // ★ BYOK 通道标记：恢复端费用显示 ' BYOK' 后缀
        byokRoute: ag._floorByokRoute || '',    // 'direct' | 'relay'
        ...
        clockTiming: ag._lastFloorTimingRecord || (ag._floorStartPerf ? { durationMs: Math.round(Math.max(0, Date.now() - ag._floorStartPerf)), networkMs: (ag._floorTiming && ag._floorTiming.networkMs) || 0, aiMs: (ag._floorTiming && ag._floorTiming.aiMs) || 0, otherMs: (ag._floorTiming && ag._floorTiming.otherMs) != null ? ag._floorTiming.otherMs : Math.max(0, (Math.max(0, Date.now() - ag._floorStartPerf)) - (((ag._floorTiming && ag._floorTiming.networkMs) || 0) + ((ag._floorTiming && ag._floorTiming.aiMs) || 0))), floorIndex: ag._currentFloorNum || 0 } : null),
