...
const i18n = (function () {
    ...
    // ── 跨窗口实时切换（独立窗口如 timeline / git-diff：同源异窗收 localStorage storage 事件）──
    // 主窗口 setLang 写 LS → 其他窗口收到事件 → 本窗跟进同款切换（本窗自己写 LS 不触发本窗事件，无环）
    try {
        window.addEventListener('storage', function (e) {
            if (e.key !== LS_KEY || !e.newValue) return;
            if (!LANG_ENABLED[e.newValue] || e.newValue === _currentLang) return;
