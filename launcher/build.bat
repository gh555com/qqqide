@echo off
REM build.bat — compile qqqide.exe + slave.exe with embedded icon
REM Uses MinGW-w64 from E:\s\d\gw\mingw64\bin\
REM Output: qqqide.exe (64-bit, ~123KB, with shell/icon.ico embedded)
REM          slave.exe (legacy compat stub, same icon)

set GW=E:\s\d\gw\mingw64\bin

echo [1/3] windres resource.rc -^> resource.o
...
echo [3/3] gcc -^> slave.exe
"%GW%\gcc.exe" -mwindows -municode -O2 -s -o slave.exe slave.c resource.o
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

echo.
echo === DONE: qqqide.exe + slave.exe with embedded icon ===
